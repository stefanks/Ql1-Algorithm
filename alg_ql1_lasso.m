function varargout = alg_ql1_lasso(problem,varargin)
%% Universal Algorithm
%
%   A method for finding an optimal solution to
%       min (1/2)*norm(B*x-y)^2 + norm(tau.*x,1)
%
...
%
%   Optional:
%       problem.normA            - An upper bound on the norm of matrix B'*B
%                                  (default = 1e6)
...
%       opts.maxMV               - Maximum number of B'*B calls allowed
%                                  (default = 10000)
...
%       numMV                    - Total work in term of B'*B calls
...
%       MVTypes                  - What were B'*B calls used for:
...
%       CGmvcount                - B'*B calls for each CG cycle
...
%     3rd argument: (optional and expensive! Only for debugging. Computed after each B'*B call)
...
Ax = @(x)problem.BT(problem.B(x));
b = problem.BT(problem.y);
tau = problem.tau;
