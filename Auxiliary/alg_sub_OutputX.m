...
if nargin==9
    ...
    fprintf('%6d %5s %13.3e %13.3e %6i             %6i             %6i\n' ,(2*numA+numB)/2, varname, optimalityMeasure(g,b,tau,x), ql1_fValue(g,b,tau,x), size(find(x<0),1),size(find(x==0),1),size(find(x>0) ,1));
    outX =x;
else
    xHistory=varargin{1};
    ...
    if norm(x-xPrevOutput)<1e-16 && size(find(sign(x)-sign(xPrevOutput)==0),1) == size(x,1)
        ...
        fprintf('%6d %5s %13.3e %13.3e %6i' ,(2*numA+numB)/2, varname, optimalityMeasure(g,b,tau,x), ql1_fValue(g,b,tau,x), size(find(x<0),1));
